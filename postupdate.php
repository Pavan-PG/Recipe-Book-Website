...
<h1 class="w3-center w3-white w3-round-large"><?php
                        ...
                        $conn = new mysqli($servername, $username, $password, $dbname);
                            if ($conn->connect_error) {
                                ...
                                $id = "select id from userrecipe where addname='$name' AND address='$address'";
    ...
    die("Give correct credentials and update later");
    $conn->close();
}
                       $sql ="UPDATE `userrecipe` SET `id`='$gotid',`recipename`='$recipename',`steps`='$steps',`indg`='$indg',`wikilink`='$wikilink',`imglink`='$imglink',`addname`='$name',`address`='$address' WHERE id='$gotid'";
            ...
            <a href="/recipeupdelete.php"><input type="button" value="Click here to update more recipes" class="w3-button w3-green w3-round-xlarge"></a>
            </div>
            </div>
            </div>
</body>
